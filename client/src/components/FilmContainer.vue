...
<template>
    <div class="">
        <div v-if="showModal" class="bg-slate-100 container mx-auto w-1/3 shadow-2xl ">
            <div class="flex flex-col justify-center  mx-10 my-5">
                <div class="text-lg font-medium pt-5">ADD MOVIE: </div>
                <label class="mt-5">Film title</label>
                <input type="text" v-model="title" id="title" class="mt-1 py-2 px-4 border border-slate-400 rounded" placeholder="Type in a movie name">

                <label class="mt-5">Release date</label>
                <input type="number" v-model="releaseDate" class="mt-1 py-2 px-4 border border-slate-400 rounded">

                <label class="mt-5">Genre</label>
                <form action="" class="">
                <select v-model="selectedGenre" id="genres" class="mt-1 py-2 px-4 border border-slate-400 rounded w-full">
                        <option v-for="genre in moviesGeners" :value="genre.id" :key="genre.id">{{ genre.title }}</option>
                    </select>
                </form>

                <label class="mt-5">Summary</label>
                <input type="text" v-model="summary" class="mt-1 py-2 px-4 border border-slate-400 rounded" placeholder="Type in a brief summary">

                <label class="mt-5">Poster</label>
                <input type="url" v-model="image" class="mt-1 py-2 px-4 border border-slate-400 rounded" placeholder="URL">

                <label class="mt-5">Quality</label>
                <input type="text"  v-model="quality" class="mt-1 py-2 px-4 border border-slate-400 rounded" placeholder="HD | FHD | 4K">

                <label class="mt-5">Length (min)</label>
                <input type="number" v-model="length" class="mt-1 py-2 px-4 border border-slate-400 rounded">
    ...
    <div class="flex justify-end ">
        <button v-if="!showModal" class="mr-48 btn-add-movie bg-blue-400 hover:bg-blue-700 text-white font-bold py-2 px-4 border rounded-lg" @click="handleOnOffMovie">Add Movie</button>
    </div>
